...
if username not in user:
    user[username] = 0
    print('Your deposit is 0 Rupees')
else:
    print('Your deposit is', user[username], 'Rupees')

balance = int(input("enter number of coins to take: (1 coin equals 1 rupee) "))
balance = user[username] + balance
...
if input("want to deal?(y/n) ").lower() == 'y':
    ...
    while not bet_done:
        ...
        if bet_amount <= user[username]:
            bet_done = True
        else:
            print("enter a valid bet_amount.")

    user[username] -= bet_amount
    bet('dealer_hand',1)
    bet('1',1)
